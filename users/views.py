...
class CreateUserView(CreateView):
    ...
    def form_valid(self, form):
        ...
        user.is_staff = False  # Важно: не staff
        user.is_superuser = False  # Важно: не superuser
        user.save()

        # Назначаем ограниченные права
        self.assign_limited_permissions(user)

        # Отправляем email подтверждения
        self.send_verification_email(user, form.cleaned_data.get("email"))

        # Автоматически логиним пользователя (опционально)
        # login(self.request, user)

        return super().form_valid(form)

    def assign_limited_permissions(self, user):
        # Если пользователь staff - не ограничиваем права
    ...
    def setup_limited_group_permissions(self, group):
        """Настраиваем права для группы Limited Users"""

        # Права для модели Reservation
        ...
        # Права для модели User (только просмотр и изменение своего профиля)
        user_content_type = ContentType.objects.get_for_model(User)
        user_perms = Permission.objects.filter(
            content_type=user_content_type,
            codename__in=['view_user', 'change_user']  # базовые права пользователя
        )
